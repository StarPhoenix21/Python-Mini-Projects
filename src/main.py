...
def resource_path(relative_path):
    base_path = os.path.abspath(".")  # Works whether running as a script or from the bundled executable
...
def blur_bg(image):
    ...
    if cached_blurred_bg is None:
        image_bgr = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
        blurred_image = cv2.GaussianBlur(image_bgr, (15, 15), 0)
        cached_blurred_bg = cv2.cvtColor(blurred_image, cv2.COLOR_BGR2RGB)
    return cached_blurred_bg
...
def victory_screen(winner_img):
    ...
    while pygame.time.get_ticks() - start_time < ROUND_OVER_COOLDOWN or pygame.key.get_pressed()[pygame.K_SPACE]:
...
def draw_gradient_text(text, font, x, y, colors):
    ...
    # Draw final, centered text
    img = font.render(text, True, colors[-1])
    screen.blit(img, (x, y))
...
def main_menu():
    ...
    while True:
        ...
        scale_factor = 1 + 0.03 * math.sin(elapsed_time * math.pi)  # Reduced oscillation for smoother effect
...
def reset_game():
    ...
    fighter_1 = Fighter(1, SCREEN_WIDTH // 4, SCREEN_HEIGHT // 2, False, WARRIOR_DATA, warrior_sheet,
                        WARRIOR_ANIMATION_STEPS, sword_fx)
    fighter_2 = Fighter(2, 3 * SCREEN_WIDTH // 4, SCREEN_HEIGHT // 2, True, WIZARD_DATA, wizard_sheet,
                        WIZARD_ANIMATION_STEPS, magic_fx)


def draw_health_bar(health, is_left, y):
    bar_width = 400
    bar_height = 40
    padding = 20
    ...
    if is_left:
        ...
        x = SCREEN_WIDTH - bar_width - padding

    pygame.draw.rect(screen, BLACK, (x, y, bar_width, bar_height))
    if health > 0:
        pygame.draw.rect(screen, RED, (x, y, health * 4, bar_height))
    pygame.draw.rect(screen, WHITE, (x, y, bar_width, bar_height), 4)


def countdown():
    countdown_font = pygame.font.Font(resource_path("assets/fonts/turok.ttf"), SCREEN_HEIGHT // 10)
...
def game_loop():
    ...
    while True:
        ...
        warrior_text = f"WARRIOR HEALTH: {fighter_1.health}"
        wizard_text = f"WIZARD HEALTH: {fighter_2.health}"

        warrior_text_width = score_font.size(warrior_text)[0]
        wizard_text_width = score_font.size(wizard_text)[0]

        draw_text(warrior_text, score_font, RED, 20, 20)
        draw_text(wizard_text, score_font, RED, SCREEN_WIDTH - wizard_text_width - 20, 20)

        draw_health_bar(fighter_1.health, True, 50)
        draw_health_bar(fighter_2.health, False, 50)
